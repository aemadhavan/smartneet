...
export default function ZoologyContent({ initialTopics }: ZoologyContentProps) {
  ...
  const practiceLink = useMemo(() => {
    return isPremium ? "/practice?subject=zoology" : "/practice?subject=zoology&limit=free";
  }, [isPremium]);

  // Memoize topics with access info
  const topicsWithAccess = useMemo(() => {
    return initialTopics.map((topic, index) => ({
      ...topic,
      isAccessible: isPremium || index < 2
    }));
  }, [initialTopics, isPremium]);

  // Show minimal loading state without blocking content
  const showContent = !subscriptionLoading || initialTopics.length > 0;
    ...
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-8">
        ...
        <div className="grid md:grid-cols-3 gap-4 text-center">
          ...
          <div className="bg-amber-50 p-4 rounded-md">
            <p className="text-amber-600 text-2xl font-bold mb-1">{initialTopics.length}</p>
      ...
      {subscriptionLoading && initialTopics.length === 0 && (
