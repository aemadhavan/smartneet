//File: src/app/physics/topics/[id]/page.tsx
...
export default function PhysicsTopicDetailPage() {
  ...
  useEffect(() => {
    const checkAccessAndFetchData = async () => {
      ...
      try {
        ...
        const physicsSubjectId = 1; // Physics subject ID
        const topicsResponse = await fetch(`/api/topics?subjectId=${physicsSubjectId}&isRootLevel=true&isActive=true`);
        ...
        if (isPremiumTopic && !isPremium) {
          ...
          router.push(`/pricing?from=physics-topic-${topicId}`);
          return;
        }
    ...
    return (
      <div className="container mx-auto py-8 px-4">
        <div className="bg-red-50 border border-red-200 text-red-800 rounded-md p-4 text-center">
          ...
          <Link href="/physics" className="mt-4 inline-block bg-red-100 text-red-800 px-4 py-2 rounded hover:bg-red-200">
            Return to Physics
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-7xl">
      {/* Navigation */}
      <div className="mb-8">
        <Link href="/physics" className="text-indigo-600 hover:text-indigo-800 flex items-center text-sm">
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 sticky top-4">
            ...
            <div className="mt-6 pt-4 border-t">
              ...
              {Boolean(isPremium) || isFreeTopic ? (
                <Link
                  href={`/practice?subject=physics&topicId=${topic.topic_id}`}
                  className="bg-blue-600 text-white w-full py-2 px-4 rounded-md hover:bg-blue-700 flex items-center justify-center"
                >
                ...
                <Link
                  href={`/pricing?from=practice-physics-${topic.topic_id}`}
          ...
          {activeSubtopic ? (
            <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
              ...
              <div className="mt-6">
                ...
                <ul className="space-y-2">
                  <li className="flex items-start">
                    <CheckCircle size={20} className="text-blue-500 mr-2 mt-0.5 flex-shrink-0" />
              ...
              <div className="mt-8 flex justify-end">
                ...
                {Boolean(isPremium) || isFreeTopic ? (
                  <Link
                    href={`/practice?subject=physics&topicId=${topic.topic_id}&subtopicId=${activeSubtopic.subtopic_id}`}
                    className="bg-blue-50 text-blue-700 hover:bg-blue-100 px-4 py-2 rounded-md"
                  >
              ...
              {allTopics.slice(0, 2).map((relatedTopic) => (
                relatedTopic.topic_id.toString() !== topicId && (
                  <Link
                    ...
                    href={`/physics/topics/${relatedTopic.topic_id}`}
