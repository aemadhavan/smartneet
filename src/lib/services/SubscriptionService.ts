...
export class SubscriptionService {
  ...
  async canUserTakeTest(userId: string): Promise<{ canTake: boolean; reason?: string }> {
    ...
    try {
      ...
      if (plan.test_limit_daily !== null && plan.test_limit_daily !== undefined) {
        const dailyLimit = plan.test_limit_daily as number;
        if ((updatedSubscription.tests_used_today ?? 0) >= dailyLimit) {
          ...
          return {
            canTake: false,
            reason: `You've reached your daily limit of ${dailyLimit} tests. Upgrade to premium for unlimited tests. ${message}`
          };
        }
      }
